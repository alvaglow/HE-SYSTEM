/**
 * Mirrors apps/web/app/student/timetable (StudentTimetablePage). No
 * join_url deep-link handling since HE-SYSTEM doesn't bundle an in-app
 * webview for online classes — mobile opens the join link externally.
 */
...
export default function StudentTimetableScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const enrollments = (enrollmentsRaw ?? []) as unknown as Array<{ classes: ClassRow | null }>
    const classes = enrollments.map(e => e.classes).filter((c): c is ClassRow => !!c && !c.is_cancelled && c.starts_at >= nowIso)
      .sort((a, b) => a.starts_at.localeCompare(b.starts_at)).slice(0, 50)

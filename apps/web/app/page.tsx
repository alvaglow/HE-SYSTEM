        ...
        <div className="text-center">
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            ...
            <Link
              ...
              className="rounded-full bg-red-600 px-8 py-3 text-sm font-semibold text-white shadow-lg shadow-red-500/30 transition-all hover:bg-red-500"
            >
              Admin Portal
            </Link>
          </div>
...
function PortalCard({ title, description, href, color, icon }: { title: string; description: string; href: string; color: string; icon: string }) {
  return (
    <Link
      href={href}
      className="group relative overflow-hidden rounded-2xl bg-slate-800/50 p-6 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:bg-slate-800/70 hover:shadow-xl hover:shadow-slate-900/20 border border-slate-700/50"
    >

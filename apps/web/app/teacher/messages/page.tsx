...
export default async function TeacherMessagesPage() {
  ...
  const [{ data: sentRaw }, { data: receivedRaw }, { data: classesRaw }] = await Promise.all([
    ...
    supabase.from('classes').select('class_enrollments(students(user_id, users(full_name)))').eq('teacher_id', teacher?.id ?? ''),
  ])
  ...
  const recipients = [...studentMap.entries()].map(([id, label]) => ({ id, label }))

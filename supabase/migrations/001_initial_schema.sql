...
CREATE TABLE kpi_records (
  ...
  pillar1_score   NUMERIC(5,2),
  pillar2_score   NUMERIC(5,2),
  pillar3_score   NUMERIC(5,2),
  pillar4_score   NUMERIC(5,2),
...
-- =============================================================================
-- TRIGGER — partner tier auto-update
-- =============================================================================
...
-- users
CREATE POLICY "users: own row" ON users FOR SELECT USING (id = auth.uid());
...
-- students
CREATE POLICY "students: own record" ON students FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "students: institution view" ON students FOR SELECT
  USING (institution_id = get_my_institution_id() AND get_my_role() IN ('teacher', 'admin', 'management'));
...
CREATE POLICY "attendance: parent sees child" ON attendance_records FOR SELECT
  USING (institution_id = get_my_institution_id()
    AND student_id IN (SELECT student_id FROM parent_student_links WHERE parent_user_id = auth.uid()));
...
-- wallets
CREATE POLICY "wallets: own" ON digital_wallets FOR SELECT USING (user_id = auth.uid());
...
-- kpi
CREATE POLICY "kpi: own" ON kpi_records FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "kpi: management" ON kpi_records FOR SELECT
  USING (institution_id = get_my_institution_id() AND get_my_role() = 'management');
...
-- payroll
CREATE POLICY "payroll: own" ON payroll_records FOR SELECT USING (user_id = auth.uid());
...
CREATE POLICY "messages: read own" ON messages FOR UPDATE USING (recipient_id = auth.uid());

-- announcements
CREATE POLICY "announcements: read published" ON announcements FOR SELECT
  USING (institution_id = get_my_institution_id() AND is_published = TRUE);
